    ...
    <div className="min-h-[100dvh] w-full">
      <main className="mx-auto grid w-full max-w-[860px] gap-5 p-4 sm:gap-4 md:p-8">
        <header className="rounded-2xl border border-slate-400/30 bg-gradient-to-br from-slate-900/65 to-slate-800/35 p-5 backdrop-blur-[2px] sm:rounded-[14px] sm:p-4">
          <h1 className="text-hero">{"Martin Silva\u2011Diaz"}</h1>
          <p className="mt-2 text-subheading text-slate-300">
        ...
        >
          {externalLinks.map((link) => (
            <a
              key={link.id}
              href={link.href}
              className="inline-flex items-center gap-2 rounded-full border border-slate-400/35 bg-gradient-to-br from-slate-800/75 to-slate-700/52 px-4 py-2 text-caption text-slate-200 no-underline transition-all duration-150 ease-in hover:border-slate-400/80 sm:min-h-11 sm:px-3.5 sm:py-2.5"
              target="_blank"
              rel="noreferrer"
            >
              <img
